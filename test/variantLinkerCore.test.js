...
describe('variantLinkerCore.js', () => {
  ...
  describe('Integration with VCF, Recoder, and VEP', () => {
    ...
    it('should detect, process, and annotate the ENST00000302118:c.137G>A variant correctly', async function() {
      ...
      fetchApiStub.withArgs(sinon.match(/variant_recoder\/ENST00000302118:c.137G>A/))
        .resolves({
      ...
      fetchApiStub.withArgs(sinon.match(/vep\/homo_sapiens\/region/))
        .resolves([{
      ...
      try {
        ...
        expect(result.annotationData[0]).to.have.property('originalInput', 'ENST00000302118:c.137G>A');
        expect(result.annotationData[0].transcript_consequences[0]).to.have.property('transcript_id', 'ENST00000302118');
      } finally {
        // Clean up
        fetchApiStub.restore();
      }
