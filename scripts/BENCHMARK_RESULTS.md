...
> Historical results from the previous benchmark implementation. Its timing included a fixed delay and forced diagnostic logging, so these values are not comparable with the corrected runner. Generate new results with `npm run benchmark -- --repeat 3 --format json`; this uses synthetic offline fixtures by default. Add `--live` explicitly to measure public Ensembl requests.
